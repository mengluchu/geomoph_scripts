...
def run(k, j = 0, filename = "/Volumes/Meng_Mac/obia/temp/HP1103.gpkg",resultdir = "/Volumes/Meng_Mac/obia/results/"):
...
# %% [code]
    ...
    #    X_train.dtypes
        ...
        plt.colorbar()
        fname = f"{filedir}/cmlog_{layers[j]}_{classes[k]}"
        plt.savefig(fname, dpi=1200)
        # %% [code]
        fig, ax = plt.subplots(1,1)
        ...
        plt.colorbar() 
        fname = f"{filedir}/cm_tp_alltrue_{layers[j]}_{classes[k]}"
        plt.savefig(fname, dpi=1200)
        # TP/ all True
        ...
        plt.colorbar() 
        fname = f"{filedir}/cm_tp_allpred_{layers[j]}_{classes[k]}"
        plt.savefig(fname, dpi=1200)
        '''
    # TP/ (all predicted T), most of the times, the predictions. 
for j in range(len(layers)-1):
    for k in range(5):
        run(k,j, "/Volumes/Meng_Mac/obia/temp/HP1103.gpkg")
